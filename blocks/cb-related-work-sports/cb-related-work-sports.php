<?php
/**
 * CB Related Work SPORTS Block Template
 *
...
// Get service terms from current post.
// Theme is always 'Sports' for this block.
...
// Build theme ID for Sports theme
$theme_id = null;
...
// Run query if theme_id is found (Sports block always needs theme)
if ( $theme_id ) {
	$posts = array();
	...
	if ( count( $posts ) < 4 ) {
		...
		// Add service filter only if service_id exists
		if ( $service_id ) {
			$tax_query[] = array(
				'taxonomy' => 'service',
				'field'    => 'term_id',
