...
function add_child_theme_textdomain() {
	load_child_theme_textdomain( 'cb-identity2025', get_stylesheet_directory() . '/languages' );
}
...
// Sync ACF field group JSON to blocks/ when saved.
add_action(
	'save_post',
	function ( $post_id ) {
		// Only run for ACF field groups.
		...
		$json_file    = $field_group['key'] . '.json';
		$source       = $acf_json_dir . $json_file;

		// Normalize block folder name: extract after last underscore, replace underscores with hyphens.
		if ( preg_match( '/group_(.+)$/', $field_group['key'], $matches ) ) {
			$block_name = str_replace( '_', '-', $matches[1] );
			$block_path = $blocks_dir . $block_name . '/' . $json_file;
			if ( file_exists( $source ) ) {
				if ( ! file_exists( $blocks_dir . $block_name ) ) {
					mkdir( $blocks_dir . $block_name, 0755, true );
				}
				if ( ! copy( $source, $block_path ) ) {
					error_log( 'ACF JSON sync failed: could not copy ' . $source . ' to ' . $block_path );
				}
			} else {
				error_log( 'ACF JSON sync failed: source file not found ' . $source );
			}
		} else {
			error_log( 'ACF JSON sync failed: could not parse block name from key ' . $field_group['key'] );
		}
	}
);
