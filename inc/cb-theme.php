...
add_filter( 'gform_form_tag', function( $form_tag, $form ) {
    ...
    // add Salesforce ID as data attribute
    if ( strpos( $form_tag, 'data-salesforce-id=' ) === false ) {
        $form_tag = str_replace(
            '<form ',
            '<form data-salesforce-id="MC6FXNIUWZ5ZEPLBVQKCLTBY2H5U" ',
            $form_tag
        );
    }

    // add UMA flag if required by Salesforce
    if ( strpos( $form_tag, 'data-uma-forms=' ) === false ) {
        $form_tag = str_replace(
            '<form ',
            '<form data-uma-forms="true" ',
